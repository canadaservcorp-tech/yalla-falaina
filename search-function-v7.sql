-- v7 adds one output column, `founding`: true when the provider holds one of the 50 founding
-- numbers (users.founding_number, handed out at subscription activation). Everything else
...
returns table (
  ...
  subscribed boolean, boosted boolean, founding boolean, contactable boolean, distance_m double precision
)
language sql stable as $$
  with matched as (
    select distinct p.user_id, p.display_name, p.bio, p.city, p.neighbourhood,
           ...
           (u.founding_number is not null) as is_founding,
  ...
  select user_id, display_name, bio, city, neighbourhood, languages, availability,
         ...
         (is_founding and subscribed) as founding,
