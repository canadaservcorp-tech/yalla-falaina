...
**Current state (as of 2026-09-13):** Phase 1 MVP merged and Steps 1, 5, 6, 7 below are all done — concierge live in production on real Supabase + Anthropic (`ANTHROPIC_API_KEY` set on Railway 2026-09-13, `/api/health` confirms `concierge:true`, demo mode off), seeded jobs feed, age/terms-gated signup, error-code contract wired end-to-end, informal-listing moderation UI shipped, the Section 10 profile-completeness intake gate shipped, `DEPLOY.md` + `GO-LIVE.md` (the go-live runbook) both written. Payments now run on two rails: PayPal (original) **and** Stripe (added Sept 2026 — see Step 8). Brand rename is DONE and merged: `devin/1788914730-rebrand-yalla-nsafer` (Latin-script "Yalla Falaina" → "Yalla Nsafer") merged into `main` 2026-09-09 — "Yalla Nsafer" (Arabic يلا نسافر) is now the name everywhere, code and docs alike. `PAYWALL_ENFORCED` is still `false` in production pending Hicham's own go-live call — the remaining pre-launch items (a real Adzuna/Jooble job-feed key, `STRIPE_*`, a verified Resend sending domain) are ops/credential steps, not code.

A hardening sweep (2026-09-09) closed out several latent bugs found by auditing the write paths, none of which touch `lib/yf/systemPrompt.js` or matching semantics: a `seeker_profiles.profile_id` TOCTOU race (unique constraint + real upsert — `schema.sql`, `lib/profileWrite.js`); the same race in `lib/usage.js`'s daily-quota counter and `routes/concierge.js`'s free-preview counter (two new atomic Postgres RPCs, `usage_charge()`/`increment_free_preview()`, same style as the existing `rate_hit()`); silent row-truncation in the three scheduled jobs (`scripts/document-retention.js`, `scripts/subscription-lapse.js`, `scripts/profile-retention.js`) now paged via a shared `lib/paginate.js` helper instead of one unbounded `.select()`; a non-atomic `jobs`-insert-then-status-update in `routes/admin-informal-listings.js`'s approval flow that could strand a submission at `pending` forever on a partial failure; and full test coverage added for `lib/prohibited-categories.js` (the Section 6.6 keyword gate), which had none.

Two more hardening rounds followed, both on the intake-extraction contract (`routes/concierge.js`'s `intakeInstructions`/`PROFILE_BLOCK_RE` handling), after live testing and a follow-up audit found the model's `---PROFILE---` contract had real failure modes: the block is now required unconditionally on every intake reply (not conditionally "when something new was learned"), restates every known field each turn, goes block-first so a token-budget cutoff only ever costs trailing prose, and the no-jobs rule explicitly forbids naming example sectors/roles. Two raw-fencing leaks were found and fixed on top of that — a truncated/unclosed block, and (found in a later follow-up audit) a block that matches successfully but constitutes the model's *entire* reply — both now fall back to a clean line instead of ever showing the seeker raw internal JSON. Also shipped: a PayPal setup-script safety guard (refuses to register a live webhook at a non-`https://` `PUBLIC_URL`) with full test coverage, and test coverage for the one `PAYWALL_ENFORCED=true` + active-subscriber combination the suite had never exercised. **265/265 tests pass.**
