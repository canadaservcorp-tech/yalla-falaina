-- v5 adds two seeker filters on top of v4: "available now" and languages spoken.
-- Both are applied after the subscription rules, so a lapsed provider is never surfaced by the
-- "available now" filter (its availability is withheld exactly like its distance).
