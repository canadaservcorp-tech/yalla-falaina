...
const JOOBLE_COUNTRY_ALIASES = {
  ...
  // Turkey is a work/study/medical destination in its own right (see geo.js's
  // TR note) -- Jooble posts it under any of these spellings or city names.
  turkey: 'Turkey', turkiye: 'Turkey', 'türkiye': 'Turkey',
  istanbul: 'Turkey', 'i̇stanbul': 'Turkey', ankara: 'Turkey', izmir: 'Turkey', 'i̇zmir': 'Turkey',
};
...
async function refresh() {
  ...
  // Must run BEFORE the upsert loop below: an upsert can't tell "this row is
  // brand new" from "this row already existed and just got refreshed" after
  // the fact, so lib/jobAlerts.js has to see the table's state first.
  const newRows = await jobAlerts.findNewRows(rows).catch(e => { console.error('jobs-ingest: findNewRows', e.message); return []; });
  ...
  // Best-effort and never blocking: a notification hiccup must not turn a
  // successful ingest run into a failed one.
  if (newRows.length) {
    const sent = await jobAlerts.notifyNewJobs(newRows).catch(e => { console.error('jobs-ingest: notifyNewJobs', e.message); return 0; });
    if (sent) console.log(`jobs-ingest: ${sent} new-job push notification(s) sent for ${newRows.length} new job(s)`);
  }
  return n;
}
