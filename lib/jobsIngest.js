...
// the operator's own JOB_API_LOCATION (the country/region this deploy is
// actually configured to query for) -- returns the first that resolves via
// the alias table above, or null if none do. A null means the row's country
// stays whatever raw text Jooble sent (same as before this fix -- logged as
// a real gap rather than silently guessed at, per Section 6.1's tracks
// driving matching).
function joobleCountryFor(locationText) {
  const parts = String(locationText || '').split(',').map(s => s.trim()).filter(Boolean);
  const candidates = [parts[parts.length - 1], locationText, process.env.JOB_API_LOCATION];
...
async function fetchJooble() {
  ...
  const j = await r.json();
  return (j.jobs || []).map(x => {
    const resolvedCountry = joobleCountryFor(x.location);
