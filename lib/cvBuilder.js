...
function buildCvData({ user, profile, seekerProfile, lang, variant }) {
  const p = profile || {};
  const sp = seekerProfile || {};
  // 'student' = academic CV (education leads, work follows) for seekers on
  // the study track; 'professional' (default) = the job-seeker layout.
  const v = variant === 'student' || variant === 'professional' ? variant
    : p.seeking_study ? 'student' : 'professional';
  return {
    lang: lang === 'fr' ? 'fr' : 'en',
    variant: v,
    ...
    headline: p.role_type || p.sector
      || (v === 'student' ? [p.target_degree_level, p.target_field_of_study].filter(Boolean).join(' — ') : ''),
