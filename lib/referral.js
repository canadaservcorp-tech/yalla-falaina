...
async function creditConversionIfNew(user, patch) {
  ...
  try {
    ...
    // On error we mark nothing: the patch then lands without
    // referral_credited, and the provider's automatic webhook retry calls us
    // again to finish the grant exactly once.
    ...
    if (error) { console.error('referral credit', error.message); return; }
