...
function scoreOpportunity(opp, queryTokens) {
  const description = String(opp.requirements || opp.eligibilityNote || '').replace(/<[^>]*>/g, ' ').slice(0, MAX_DESCRIPTION_CHARS);
  const haystack = [
...
async function loadStudyOpportunities() {
  ...
  const { data, error } = await supabase.from('study_opportunities')
    .select('id, kind, title, institution, country, city, degree_level, field_of_study, language, tuition_note, eligibility_note, deadline, requirements, source_type, source_url')
    .eq('status', 'active')
...
// filter, same "preference, not a wall" rule as job matching.
...
module.exports = { retrieveStudyOpportunities, loadStudyOpportunities, toPromptOpportunity, scoreOpportunity };
