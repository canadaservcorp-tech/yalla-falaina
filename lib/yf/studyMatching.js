...
function scoreOpportunity(opp, queryTokens) {
  ...
  // durationNote is short and first-class — put it in the uncapped haystack
  // beside degreeLevel/fieldOfStudy, NOT inside the capped description where a
  // long requirements field could push it past the slice and make duration
  // queries score zero.
  const haystack = [
    opp.title, opp.institution, opp.country, opp.city,
    opp.degreeLevel, opp.fieldOfStudy, opp.durationNote, description,
  ].join(' ');
...
function toPromptOpportunity(row) {
  return {
    ...
    durationNote: row.duration_note || '', // e.g. "2 years full-time" -- how long the program runs
...
async function loadStudyOpportunities() {
  ...
  const { data, error } = await supabase.from('study_opportunities')
    .select('id, kind, title, institution, country, city, degree_level, field_of_study, language, duration_note, tuition_note, funding_coverage_pct, eligibility_note, deadline, requirements, source_type, source_url')
    .eq('status', 'active')
