...
// Bounds tokenization cost against a real-world description that can run to
// several paragraphs — scoreJob only cares about token PRESENCE (jobTokens
// is a Set), so a long description can't inflate a match by repetition
// either way; this cap exists purely so one unusually long listing can't
// make every scoring pass noticeably slower.
const MAX_DESCRIPTION_CHARS = 2000;

function scoreJob(job, queryTokens) {
  // `job.keywords` is how the ORIGINAL seed/prototype data got real matching
  // signal beyond a short title — the handoff bundle hand-curated a keywords
  // array per listing (lib/jobsIngest.js's fetchSeed). Adzuna/Jooble never
  // send anything resembling that field, so a live Adzuna feed scored
  // against title/category/country/city/keywords alone was effectively
  // scoring against title/category/country/city only — real postings often
  // have terse or jargon-y titles ("CDL-A Driver, sign-on bonus") where the
  // actual sector/skill words a seeker's query would hit only appear in the
  // full description. `job.requirements` (that description) is included
  // here so live feeds get real signal instead of relying on a field that
  // only ever existed for curated seed data. Stripped of any HTML tags a
  // provider's description might contain (a bare split on non-letters would
  // otherwise tokenize tag names as if they were content words).
  const description = String(job.requirements || '').replace(/<[^>]*>/g, ' ').slice(0, MAX_DESCRIPTION_CHARS);
  const haystack = [
    job.title,
    job.category,
    job.country,
