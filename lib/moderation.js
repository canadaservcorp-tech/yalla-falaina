...
async function checkImage(base64) {
  ...
  // only VERY_LIKELY acts: at LIKELY, SafeSearch flags things like classical nude paintings
  const unsafe = a.adult === 'VERY_LIKELY' || a.racy === 'VERY_LIKELY';
  return { safe: !unsafe, checked: true, adult: a.adult, racy: a.racy };
}

// An image hit blocks the photo and records a strike for human review — it never bans on its own,
// because SafeSearch has false positives and a ban forfeits a paid subscription.
const STRIKES_BEFORE_BAN_ADVICE = 3;

async function strikeImage(userId, where, verdict, conversationId) {
  const { count } = await supabase.from('reports')
    .select('id', { count: 'exact', head: true })
    .eq('target_user_id', userId).eq('kind', 'image_strike').neq('status', 'dismissed');
  const strike = (count || 0) + 1;
  const levels = `adult=${verdict.adult || '?'} racy=${verdict.racy || '?'}`;
  const advice = strike >= STRIKES_BEFORE_BAN_ADVICE
    ? ` — strike ${strike}: review for permanent ban` : '';
  const { error } = await supabase.from('reports').insert({
    reporter_id: null, conversation_id: conversationId ?? null, target_user_id: userId,
    kind: 'image_strike', status: 'open',
    reason: `image blocked in ${where} (${levels}); strike ${strike}${advice}`,
  });
  if (error) console.error('strikeImage', error);
  return { strike, banRecommended: strike >= STRIKES_BEFORE_BAN_ADVICE };
}
...
module.exports = { checkImage, checkText, flagText, blockUser, strikeImage, STRIKES_BEFORE_BAN_ADVICE };
