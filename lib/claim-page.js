...
const T = {
  fr: {
    ...
    openUntil: (until, days) => `Mois de lancement : votre fiche est joignable gratuitement par les clients jusqu’au ${until} (${days} jours restants). Ensuite, seules les fiches réclamées peuvent être contactées.`,
    waiting: n => n === 1
      ? '1 demande de client attend sur cette fiche. Réclamez-la pour la lire.'
      : `${n} demandes de clients attendent sur cette fiche. Réclamez-la pour les lire.`,
  ...
  en: {
    ...
    openUntil: (until, days) => `Launch month: clients can reach your listing for free until ${until} (${days} days left). After that, only claimed listings can be contacted.`,
    waiting: n => n === 1
      ? '1 client request is waiting on this listing. Claim it to read it.'
      : `${n} client requests are waiting on this listing. Claim it to read them.`,
...
// The window and the waiting count are facts or nothing: an unknown count prints no line.
function openAccessLines(t, window_, waiting) {
  if (!window_ || !window_.active || !window_.until) return '';
  const out = [`<p class="open">${esc(t.openUntil(window_.until, window_.daysLeft))}</p>`];
  if (Number.isFinite(waiting) && waiting > 0) out.push(`<p class="waiting">${esc(t.waiting(waiting))}</p>`);
  return out.join('\n       ');
}

// listing: { display_name, city, rbq_licence, claimed, trades: [{name_fr, name_en}] }
function render(listing, { lang = 'fr', token = null, founding = null, window: window_ = null, waiting = null } = {}) {
  const l = lang === 'en' ? 'en' : 'fr';
  const t = T[l];
  ...
  const action = listing.claimed
    ...
    : `<p class="why">${esc(t.why)}</p>
       ${openAccessLines(t, window_, waiting)}
...
.open{background:#eaf6f6;border-radius:12px;padding:12px 14px;color:#0b5b5a}
.waiting{background:#0f7c7b;color:#fff;border-radius:12px;padding:12px 14px;font-weight:700}
...
module.exports = { render, claimUrl, isLicence, PRICE, foundingLine, openAccessLines };
