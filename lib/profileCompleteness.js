...
// The one fact it can't read itself is whether a medical_intake_requests row
// exists — callers pass `hasMedicalIntake` (they fetch it only for
// seeking_treatment profiles anyway).
...
function computeCompleteness({ profile, seekerProfile, hasMedicalIntake } = {}) {
  const p = profile || {};
  ...
  // A student's "what are you aiming for" is a degree/field, not a job sector:
  // signup's student track sets seeking_study + target_* without ever asking
  // sector/role_type, so requiring those would lock every student out of
  // study matching behind an irrelevant work question. A job-seeker target
  // still also satisfies the gate for a student (the tracks aren't exclusive).
  const hasTarget = p.sector || p.role_type
    || (p.seeking_study && (p.target_degree_level || p.target_field_of_study))
    || (p.seeking_treatment && hasMedicalIntake);
  if (!hasTarget)
    missing.push(p.seeking_treatment ? 'required_treatment'
      : p.seeking_study ? 'target_degree_level_or_field_of_study' : 'sector_or_role_type');
