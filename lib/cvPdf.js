...
function renderCvPdf(data) {
  return new Promise((resolve, reject) => {
    ...
    // A student CV leads with education (an admissions reader asks "what did
    // they study" first); the professional layout leads with work experience.
    const ordered = data.variant === 'student' ? ['education', 'experience'] : ['experience', 'education'];
    for (const which of ordered) {
      if (which === 'experience' && data.workHistory.length) {
      ...
      if (which === 'education' && data.education.length) {
        section(t.education);
