...
const limits = {
  ...
  // Google sign-in can't use `credentials` above: it keys on req.body.email,
  // which an OAuth redirect has none of, so every seeker behind one carrier
  // NAT — most of the MENA audience, on mobile data — would share a single
  // 10-per-15-minutes budget. There is no secret to guess here either; the
  // ceiling exists to bound account minting, and one sign-in costs 3 hits.
  oauth: limiter('oauth', 15 * 60 * 1000, 60, 'Too many sign-in attempts — try again shortly.'),
