...
//
// `none`'s cap can never sit below the free-preview allowance: the paywall
// check (402) runs BEFORE this quota check in routes/concierge.js, so a cap
// under FREE_PREVIEW_LIMIT makes a non-subscriber hit a daily-quota 429 in
// the middle of the advertised preview and can never reach the intended
// subscription-required response (live smoke-test finding).
const FREE_PREVIEW = Number(process.env.FREE_PREVIEW_LIMIT) > 0 ? Number(process.env.FREE_PREVIEW_LIMIT) : 8;
const TIER_UNITS = {
  none: Math.max(5, FREE_PREVIEW), // a few free questions so a seeker can try the concierge
  basic: 40,
  starter: 15,
  plus: 120,
