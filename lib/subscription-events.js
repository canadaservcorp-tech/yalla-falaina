...
function accountPatch(eventType, resource = {}, now = new Date()) {
  ...
  if (ENDED.has(eventType)) {
    return { subscription_status: 'canceled', subscription_cancel_at: null, subscription_period_end: periodEnd };
  }
  if (GRACE.has(eventType)) {
    // keep the paid days; lapse now when nothing is paid for
    return { subscription_cancel_at: periodEnd && new Date(periodEnd) > now ? periodEnd : now.toISOString() };
  }
  return null;
}
