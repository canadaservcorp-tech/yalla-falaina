...
test('GET /api/subscription/status with token -> { success, status, periodEnd }', async () => {
  const token = actor(h, { id: 11, role: 'provider', extra: { subscription_status: 'active', subscription_period_end: null } });
...
test('an unverified account cannot use authenticated endpoints -> 403', async () => {
  const token = actor(h, { id: 12, role: 'provider', verified: false });
  const res = await fetch(h.base + '/api/subscription/status', { headers: auth(token) });
  assert.equal(res.status, 403);
});

test('a banned account is refused -> 403', async () => {
  const token = actor(h, { id: 13, role: 'provider', banned: true });
  const res = await fetch(h.base + '/api/subscription/status', { headers: auth(token) });
  assert.equal(res.status, 403);
});
