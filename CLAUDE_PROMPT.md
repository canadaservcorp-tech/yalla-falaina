...
- **Payments**: PayPal only (`routes/subscription.js`, `lib/paypal.js`, `scripts/paypal-setup.js`) — single $25/month "Basic" tier. **Never Stripe.** Webhook verifies via `verify-webhook-signature`; `lib/subscription-events.js` maps events.
