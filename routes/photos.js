...
// only real JPEG/PNG/WebP bytes are stored, whatever the data-URL claimed
...
router.post('/:id/photos', sec.limits.upload, async (req, res) => {
  const convId = req.params.id;
  if (!sec.isId(convId)) return res.status(400).json({ error: 'Invalid conversation' });
  ...
  if (!verdict.safe) {
    // photo is never delivered; a moderator decides on the account
    await strikeImage(req.user.id, 'chat', verdict, convId);
  ...
  const path = `${convId}/${Date.now()}_${req.user.id}.${ext}`;
  const up = await supabase.storage.from(BUCKET).upload(path, buf, { contentType: mime, upsert: false });
  if (up.error) { console.error('photo upload', up.error); return res.status(500).json({ error: 'Upload failed' }); }
